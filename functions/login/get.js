...
module.exports.handler = function(userId, pathParams, queryParams, body, done) {
    return action(queryParams.token, done);
};
...
var getCognitoId = function(callback) {
    ...
    _defaults(getIdParams, cognitoLoginsParam);
...
var getCredentials = function(callback) {
    var getCredentialsForIdentityParams = _defaults({}, cognitoLoginsParam, cognitoIdentity);
...
var updateUser = function(callback) {
    dynamodbDoc.update({
        ...
        Key : { id : userId },
        UpdateExpression : 'SET #facebookId=:facebookId, #email=:email, #profilePicture=:profilePicture, #name=:username, #picks=if_not_exists(#picks, :picks), #scores=if_not_exists(#scores, :scores), #leagues=if_not_exists(#leagues, :leagues), #isAdmin=if_not_exists(#isAdmin, :isAdmin)',
